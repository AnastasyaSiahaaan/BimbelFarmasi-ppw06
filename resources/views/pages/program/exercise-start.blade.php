...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-8">
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 rounded-xl bg-white p-6 shadow-lg sticky top-4 z-10">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600">{{ $exercise['total_questions'] }} Soal</p>
                </div>
        ...
        <!-- Form -->
        <form id="exerciseForm" action="{{ route('program.exercise.submit', ['id' => $programId, 'exerciseId' => $exercise['id']]) }}" method="POST">
            @csrf
                        ...
                        <div class="space-y-3">
                            @foreach($question['options'] as $key => $option)
                                <label class="flex items-start gap-3 p-4 rounded-lg border-2 border-gray-200 cursor-pointer transition hover:border-[#2D3C8C] hover:bg-blue-50">
                                    <input type="radio" name="answers[{{ $question['id'] }}]" value="{{ $key }}" class="mt-1 h-4 w-4 text-[#2D3C8C] focus:ring-[#2D3C8C]">
            ...
            <div class="mt-8 rounded-xl bg-white p-6 shadow-lg sticky bottom-4">
                <div class="flex items-center justify-between">
                    <a href="{{ route('program.exercises', $programId) }}" class="text-gray-600 hover:text-gray-900">
